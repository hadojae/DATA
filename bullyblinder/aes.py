...
"""
This file sourced from aes.py from here http://wiki.birth-online.de/snippets/python/aes-rijndael

it is a CTR mode of operation
"""
...
if __name__ == "__main__":
    count = 0 
    plaintext = "This old man loves to party"
    secret = hashlib.sha256("somesecret").digest()
    password = secret
    t0 = time.time()
    logging.info("hash" + hashlib.sha256("somesecret").hexdigest())
    while(count < 10000):
        count += 1
        #print count
        ciphertext = encrypt(plaintext, password, 256);
        origtext = decrypt(ciphertext, password, 256);
        #print plaintext, origtext
    print str((time.time() - t0) / 10000)
    print plaintext, origtext
